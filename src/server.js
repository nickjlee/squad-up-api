...
io.on('connection', socket => {
  ...
  socket.on('message', async function(data) {
    ...
    const response = await livechatService
      .insertChat(app.get('db'), newChat)
      .then(re => {
        return livechatService.getChat(app.get('db'), re[0].id)
      })
  ...
  socket.on('delete chat', async function(data) {
    ...
    const chat = await livechatService.findChat(app.get('db'), id)

    if (chat[0].user_id == user_id[0].id) {
      await livechatService.deleteChat(app.get('db'), id)
